...
# meta banner: https://raw.githubusercontent.com/AmoreForever/assets/master/AutoProfile.jpg
__version__ = (1, 1, 0)
...
@loader.tds
class AvtoProfilMod(loader.Module):
    """Profilingiz uchun avtomatik vaqt"""
    ...
    async def cfautoprofcmd(self, message):
        """Sozlash orqali vaqt mintaqasini belgilash"""
    ...
    async def autobiocmd(self, message):
        """Hisobingizdagi bioni joriy vaqt o'zgarishi bilan avtomatik ravishda o'zgartiradi:
         .autobio 'xabar, vaqt {vaqt}'"""
    ...
    async def autonamecmd(self, message):
        """Sizning Telegram akkountingiz nomini joriy vaqt o'zgarishi bilan avtomatik ravishda o'zgartiradi:
         .autoname '<xabar, vaqt {vaqt}>'"""
    ...
    async def stopautonamecmd(self, message):
        """Shunchaki vaqtni to'xtatish uchun .stopautoname deb yozing"""
    ...
    async def aguide(self, message):
        ...
        await utils.answer(
            message, 
            f"<emoji document_id=5789581976176430614>💸</emoji> Masalan:\n\n<emoji document_id=5789667570579672963>💸</emoji> AutoName: <code>{pref}autoname '{nick} | {time}'</code>\n"
            f"<emoji document_id=5789667570579672963>💸</emoji> AutoBio: <code>{pref}autobio 'ma'lumot | {time}'</code>\n"
        if args == "en"
        else (
